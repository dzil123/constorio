...
const fn my_game(miner: NewMiner<IronOre>) -> (Bundle<IronOre, U5, impl Unsigned>,) {
    let (miner, bundle1) = miner.mine_for_duration::<U10>();
    (bundle1,)
}

const fn my_game2(miner: NewMiner<IronOre>) -> (Bundle<IronOre, U5, impl Unsigned>,) {
    ...
    (bundle2.combine(bundle1).combine(bundle3),)
}

const fn my_game3(
    miner1: NewMiner<IronOre>,
    miner2: NewMiner<IronOre>,
) -> (Bundle<IronOre, U5, impl Unsigned>,) {
    let (miner1, bundle1) = miner1.mine_for_duration::<U6>();
    let (miner2, bundle2) = miner2.mine_for_duration::<U4>();
    (bundle2.combine(bundle1),)
}
